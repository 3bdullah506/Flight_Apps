...
class _CartScreenState extends State<CartScreen> {
  ...
  Future<void> _confirmBooking() async {
    ...
    // نجلب بيانات المستخدم (الهاتف ورقم الجواز)
    ...
    for (final item in List.from(widget.cart)) {
      for (int t = 0; t < item.quantity; t++) {
        ...
        try {
          await _firebaseService.bookFlight(order);
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('$route: ${e.toString().replaceFirst('Exception: ', '')}'),
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  child: ListTile(
                    ...
                    title: Text('${item.flight.origin} → ${item.flight.destination}',
                        style: const TextStyle(fontWeight: FontWeight.bold)),
                    subtitle: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(DateFormat('dd/MM/yyyy').format(item.flight.date)),
                        if (item.flight.departureTime.isNotEmpty)
                          Text('إقلاع: ${item.flight.departureTime}',
                              style: const TextStyle(fontSize: 11, color: Colors.grey)),
                        Text('${item.quantity} × \$${item.flight.price.toStringAsFixed(0)}',
